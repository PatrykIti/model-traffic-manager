locals {
  scope_key                 = "e2e-aks-live-inbound-auth"
  ...
  node_resource_group_name  = substr("nrg-${var.environment}-${local.scope_key}-${var.name_prefix}-${var.run_id}", 0, 80)
  router_identity_name      = substr("${local.base_name}-uai", 0, 64)
  caller_identity_name      = substr("${local.base_name}-caller-uai", 0, 64)
  aks_name                  = substr("${local.base_name}-aks", 0, 63)
  dns_prefix                = substr(replace("${var.environment}${var.name_prefix}${var.run_id}", "-", ""), 0, 40)
  openai_name_base          = substr(replace("${var.environment}${var.name_prefix}oai", "-", ""), 0, 18)
  ...
  enabled_auth_deployments = [
    for deployment in var.auth_deployments : deployment
