locals {
  scope_key                 = "e2e-aks-live-observability"
  ...
  log_analytics_name        = substr("${local.base_name}-law", 0, 63)
  application_insights_name = substr("${local.base_name}-appi", 0, 255)
  ...
  enabled_observability_deployments = [
    for deployment in var.observability_deployments : deployment
