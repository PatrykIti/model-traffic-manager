...
from app.application.deployment_limit_guard import DeploymentLimitGuard, LimitedRouteTarget
...
class RouteSharedService:
    def __init__(
        self,
        shared_service_repository: SharedServiceRepository,
    ...
    def execute(self, request: SharedServiceRequest) -> OutboundResponse:
        shared_service = self._shared_service_repository.get_shared_service(request.service_id)
        if shared_service is None:
            raise SharedServiceNotFound(
                f"Shared service '{request.service_id}' was not found in the registry."
            )
        if shared_service.transport is not SharedServiceTransport.HTTP_JSON:
            raise SharedServiceTransportMismatchError(
                f"Shared service '{request.service_id}' does not support the HTTP/JSON route."
            )
        if shared_service.access_mode is SharedServiceAccessMode.DIRECT_BACKEND_ACCESS:
            raise SharedServiceExecutionDisabledError(
                f"Shared service '{request.service_id}' is configured for direct backend access."
            )

        limit_target = cast(LimitedRouteTarget, shared_service)
        lease_id = self._deployment_limit_guard.acquire(
            limit_target,
            request_id=request.request_id,
            endpoint_kind="shared_service",
        )
        try:
            if shared_service.routing_strategy is SharedServiceRoutingStrategy.TIERED_FAILOVER:
                return self._execute_with_failover(shared_service, request)
            return self._execute_single_upstream(shared_service, request)
        finally:
            self._deployment_limit_guard.release(shared_service.id, lease_id)

    def _execute_single_upstream(
        self,
        shared_service: SharedService,
        request: SharedServiceRequest,
    ) -> OutboundResponse:
        upstream = shared_service.primary_upstream
        headers = self._auth_header_builder.build(upstream.auth)
        decision_reason = "selected_shared_service_single_endpoint"
        failover_reason = (
            "provider_managed_availability"
            if shared_service.provider_managed_availability
            else None
        )
        self._record_event(
            RuntimeEvent(
                ...
                endpoint_kind="shared_service",
                deployment_id=shared_service.id,
                request_id=request.request_id,
                **upstream_runtime_metadata(upstream),
                attempt=1,
                selected_tier=upstream.tier,
        ...
        self._record_event(
            RuntimeEvent(
                ...
                **upstream_runtime_metadata(upstream),
                ...
                decision_reason=decision_reason,
                failover_reason=failover_reason,
                outcome="success" if response.status_code < 400 else "upstream_response",
    ...
    def _execute_with_failover(
        ...
        states = self._load_states(shared_service.id, shared_service.upstreams)
        ...
        for attempt in range(1, self._max_attempts + 1):
            selection_result = self._select_candidate(
                deployment_id=shared_service.id,
                upstreams=shared_service.upstreams,
                states=states,
            ...
            try:
                ...
                if not self._can_retry_failure(
                    failure=failure,
                    attempted_upstream_ids=attempted_upstream_ids,
                    upstream_count=len(shared_service.upstreams),
                ):
                    raise
                continue
        ...
        raise UpstreamSelectionError(
            "No healthy shared-service upstream is currently available for the request."
        )
