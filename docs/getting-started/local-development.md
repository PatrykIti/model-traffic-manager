...
- `make e2e-aks-live-inbound-auth-local` runs a dedicated AKS suite for inbound API bearer-token and Entra ID auth
...
- `make e2e-aks-live-inbound-auth-local` provisions a dedicated AKS validation scope and verifies inbound bearer-token and Entra ID auth against a live protected router
...
- the current live validation matrix now includes separate profiles for smoke, chat, embeddings, load balancing, shared services, inbound auth, observability, and Redis-backed shared state
